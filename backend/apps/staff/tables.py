...
class OrganisationTable(tables.Table):
    ...
    class Meta:
        ...
        exclude = ["registration_date", "id"]


class ApprovalOrganisationTable(OrganisationTable):
    info = tables.TemplateColumn(template_name="info_button.html", verbose_name=_("Status"))
    status = tables.TemplateColumn(template_name="approval_button.html", verbose_name=_("Status"))
...
class NewsletterTable(tables.Table):
    ...
    details = tables.TemplateColumn(template_name="details_button.html", verbose_name=_("Details"))


    class Meta:
        model = Newsletter
        fields = ["registration_date", "subject"]
        sequence = ("registration_date", "subject", "sending_state", "details")
        template_name = "django_tables2/bootstrap4.html"
        attrs = {
            "data-toggle": "table",
            "data-search": "false",
    ...
    def render_id(self, value):
        return format_html('<a href="/staff/view_newsletter/%s">%s</a>' % (value, _("Details")))
    ...
    def render_letterapprovedby(self, value):
        return format_html('<a href="/staff/view_newsletter/%s">%s</a>' % (value, _("Ansehen")))
